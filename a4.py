...
def is_valid_angle(ang):
    """
    Returns: True if angle is a float; False otherwise.
    
    Parameter ang: the value to check
    Precondition: float (ang must be a float)
    """
    return (type(ang) == float or type(ang) == int)
...
def is_valid_k(k):
    """
    Returns: True if k is a valid k
    
    Parameter k: the value to check
    Precondition: is an int >= 1
    """
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_k(n), report_error('n is not a valid number of sides', n)
...
def triangle_helper(p, x, y, side, d):
    ...
    if d == 0:
        ...
        small = triangle_helper(p, x, y, side, d-1)
        rest = triangle_helper(p, x, y, side/2.0, d)
...
def fill_triangle(p, x, y, side):
    """
    Fills an equilateral triangle of side length side.
